 ...
 * ```
 */
export async function createTSBlankSpace(
  ...
  // These values must be 'falsy' to not stop TypeScript's walk
  const VISIT_BLANKED = ''
  ...
  const scanner = tslib.createScanner(
    tslib.ScriptTarget.ESNext,
    /*skipTrivia: */ true,
    tslib.LanguageVariant.Standard,
  )
  ...
  function isRemovedModifier(
    kind: ts.SyntaxKind,
  ): kind is (typeof classElementModifiersToRemoveArray)[number] {
  ...
  function visitFunctionLikeDeclaration(
    ...
    if (node.modifiers) {
      visitModifiers(
        node.modifiers,
        /* addSemi */ !!nodeName && nodeName.kind === SK.ComputedPropertyName,
      )
    }
   ...
   * @returns the resulting JavaScript
   */
  function tsBlankSpace(input: string, onErrorArg?: ErrorCb): string {
    return blankSourceFile(
      tslib.createSourceFile(
        'input.ts',
        input,
        languageOptions,
